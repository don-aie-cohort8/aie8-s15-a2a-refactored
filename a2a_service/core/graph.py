...
def helpfulness_node(state: Dict[str, Any], model) -> Dict[str, Any]:
  ...
  {final_response}"""
    ...
    helpfulness_chain = helpfulness_prompt_template | model | StrOutputParser()
...
def helpfulness_decision(state: Dict[str, Any]):
    ...
    if any(
        getattr(m, "content", "") == "HELPFULNESS:END" for m in state["messages"][-1:]
    ):
        return END
...
def build_graph(model, system_instruction, format_instruction, checkpointer=None):
        ...
        if not getattr(response, "tool_calls", None):
            try:
                ...
                model_with_format = model.with_structured_output(
                    ResponseFormat, method="json_schema", include_raw=False
                )
